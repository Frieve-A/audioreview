...
周波数特性は51Hz-38kHzで、ブックシェルフスピーカーとしては許容範囲内です。実測では51Hzまでの低域再生が確認されており、測定基準表に照らすと標準的な性能です。最小インピーダンス3.8オームで86dBの感度は、駆動力の観点から課題があり、適切なアンプが必要です。高調波歪率（THD）は公開されている測定データから0.5%未満であり、スピーカーの問題レベル（0.5%以上）は回避していますが、優秀レベル（0.1%以下）には達していません。実測レビューでは800Hz-1kHzの周波数帯域で若干のギラつきが報告されており、透明性に改善の余地があります。

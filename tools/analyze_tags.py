...
PRODUCT_DIRS = {
    'ja': ROOT_DIR / '_products/ja',
    'en': ROOT_DIR / '_products/en',
}
...
category_rules_ja = {
    ...
    'productType': re.compile(r"^(ヘッドフォン|ワイヤレスヘッドホン|オーバーイヤー|ヘッドフォンアンプ|イヤフォン|完全ワイヤレスイヤホン|ワイヤレスイヤホン|有線イヤフォン|モニターイヤホン|オープンイヤー|インイヤーモニター|スピーカー|ブックシェルフスピーカー|フロア型スピーカー|スタジオモニター|ニアフィールドモニター|アクティブスピーカー|パッシブスピーカー|サウンドバー|サブウーファー|ポータブルスピーカー|デスクトップスピーカー|センタースピーカー|アンプ|プリメインアンプ|パワーアンプ|ポータブルアンプ|ストリーミングアンプ|DAC|USB DAC|デスクトップDAC|ポータブルDAC|DAC/アンプコンボ|デジタルオーディオプレーヤー|ネットワークストリーマー|ネットワークプレイヤー|レコードプレイヤー|ターンテーブル|ポータブルターンテーブル|カートリッジ|MCカートリッジ|MMカートリッジ|オーディオインターフェース|AVレシーバー|ポータブルレコーダー|2ウェイ|3ウェイ|ツイーター|Headphones?|Earphones?|IEM|TWS|Speakers?|Soundbar|Subwoofer|Amplifier|DAC|DAP|Turntable|Player|Recorder|Interface|Receiver)$", re.I),
    'technology': re.compile(r"^(平面駆動型|プラナーマグネティック|コンデンサー型|ダイナミック型|ダイナミックドライバー|バランスドアーマチュア|マルチBA|ハイブリッド構成|同軸ドライバー|骨伝導|軟骨伝導|リボン型|ホーンツイーター|チタンツイーター|アクティブノイズキャンセリング|ノイキャン|ノイズキャンセリング|オープンバック|密閉型|開放型|オープンデザイン|ワイヤレス|Bluetooth|LDAC|aptX|aptX HD|aptX Adaptive|AirPlay|DSP|ルーム音響補正|イコライザー|FIRフィルター|アダプティブEQ|バランス出力|空間オーディオ|バーチャルサラウンド|ダイレクトドライブ|ベルトドライブ|バイアンプ|D級アンプ|フルデジタル|Planar|Electrostatic|Dynamic|BA|Hybrid|Coaxial|Bone Conduction|ANC|Open-back|Closed-back|Wireless|Spatial Audio|USB-C|Dolby Atmos|DTS:X|VRM技術|SMC技術|ハーモニアス・ダイアフラム)$", re.I),
    'purpose': re.compile(r"^(コストパフォーマンス|ミッドレンジ|ハイエンド|プレミアム|エントリークラス|ゲーミング|スタジオ|モニタリング|ミキシング|マスタリング|音楽制作|レコーディング|DTM|プロ用|ポータブルオーディオ|デスクトップオーディオ|デスクトップ|ニアフィールド|スポーツ|フィット|クラシック|レトロ|低価格|高価格帯|リファレンス|レガシー製品|生産終了|限定版|革命的製品|コラボレーション|Budget|High-end|Gaming|Studio|Monitoring|Portable|Desktop|Reference)$", re.I)
}
...
def get_category(tag, lang):
    ...
    if 'Channel' in tag or 'チャンネル' in tag:
        return 'productType'
    if 'Woofer' in tag or 'ウーファー' in tag:
        return 'productType'
...
def analyze_tags():
    ...
    for lang, directory in PRODUCT_DIRS.items():
        ...
        print(f"Found {len(files)} markdown files for '{lang}'.")
